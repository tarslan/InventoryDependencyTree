...
The primary experimental environment consists of a **Windows x86_64 system using CPython 3.12.10**. Windows is selected as the primary platform because it provides the controlled and reproducible environment currently used to implement and execute the UMSAF prototype. The platform choice defines the operating-system-specific scope of the empirical results; the broader UMSAF methodology remains applicable to other operating systems through platform-specific analysis adapters.
...
* `pefile`
* strings
* Windows dependency-inspection utilities
* and related Windows binary-analysis tools.

The primary native-binary analysis targets Windows PE-format `.pyd` and `.dll` components. Linux ELF analysis is outside the primary experimental configuration and is not used as evidence for the results reported in this dissertation. A future Linux configuration may be evaluated as comparative work, but it is not required for the present UMSAF evaluation.
...
* `.so` shared objects on Linux for future comparative configurations
...
Ghidra provides a primary reverse engineering environment for examining selected native components, supplemented by `pefile`, strings, Windows dependency-inspection utilities, and other platform-appropriate binary-analysis tools. The primary analysis concerns PE-format binaries; ELF-oriented tools such as `readelf` may be used only in future comparative Linux experiments.
...
The implementation is platform-specific at the binary-analysis boundary. The primary Windows implementation fingerprints and characterizes PE-format `.pyd` and `.dll` components, while the UMSAF evidence schema and cross-layer analysis are designed to support additional binary formats in future configurations.
...
Create isolated and reproducible Windows experimental environments using the recorded CPython 3.12.10 interpreter, hash-verified dependency lockfile, virtual environments, and equivalent isolation mechanisms where appropriate.
